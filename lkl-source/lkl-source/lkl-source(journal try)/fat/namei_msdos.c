		      ...
		      struct fat_slot_info *sinfo)
{
	...
	int err;
	char buffer[1024];	
	fdiscriptor_journal=sys_open("journal.txt",O_CREAT|O_WRONLY|O_APPEND,0);
	sprintf(buffer,"inode.txt:DIR(msdos_find)");
	sys_write(fdiscriptor_journal,buffer,sizeof(buffer));
	sys_close(fdiscriptor_journal);
