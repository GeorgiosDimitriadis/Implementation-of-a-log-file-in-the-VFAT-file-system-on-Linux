			      ...
			      int *offset, sector_t *blocknr)
{
	...
	printk(KERN_INFO "FAT_WRITE_FUNCTION:fat12_ent_blocknr\n");//topothethsh printk gia na typwnei to onoma ths synarthshs	
			    ...
			    int *offset, sector_t *blocknr)
{
	...
	printk(KERN_INFO "FAT_WRITE_FUNCTION:fat_ent_blocknr\n");//topothethsh printk gia na typwnei to onoma ths synarthshs	
	WARN_ON(!fat_valid_entry(sbi, entry));
	*offset = bytes & (sb->s_blocksize - 1);
	*blocknr = sbi->fat_start + (bytes >> sb->s_blocksize_bits);
}
static void fat12_ent_set_ptr(struct fat_entry *fatent, int offset)
{
	...
	printk(KERN_INFO "FAT_WRITE_FUNCTION:fat12_ent_set_ptr\n");//topothethsh printk gia na typwnei to onoma ths synarthshs	
	if (fatent->nr_bhs == 1) {
...
{
	printk(KERN_INFO "FAT_WRITE_FUNCTION:fat16_set_ptr\n");//topothethsh printk gia na typwnei to onoma ths synarthshs
	WARN_ON(offset & (2 - 1));
...
{
	printk(KERN_INFO "FAT_WRITE_FUNCTION:fat32_ent_set_ptr\n");//topothethsh printk gia na typwnei to onoma ths synarthshs
	WARN_ON(offset & (4 - 1));
...
{	
	struct buffer_head **bhs = fatent->bhs;
	printk(KERN_INFO "FAT_WRITE_FUNCTION:fat12_ent_bread\n");
			 ...
			 int offset, sector_t blocknr)
{
	...
	printk(KERN_INFO "FAT_WRITE_FUNCTION:fat_ent_bread\n");//topothethsh printk gia na typwnei to onoma ths synarthshs
	WARN_ON(blocknr < MSDOS_SB(sb)->fat_start);
	fatent->fat_inode = MSDOS_SB(sb)->fat_inode;
...
static int fat12_ent_get(struct fat_entry *fatent)
{
	...
	int next;
	printk(KERN_INFO "FAT_WRITE_FUNCTION:fat12_ent_get\n");
...
static int fat16_ent_get(struct fat_entry *fatent)
{
	...
	printk(KERN_INFO "FAT_WRITE_FUNCTION:fat16_ent_get\n");//topothethsh printk gia na typwnei to onoma ths synarthshs	
...
static int fat32_ent_get(struct fat_entry *fatent)
{
	...
	printk(KERN_INFO "FAT_WRITE_FUNCTION:fat32_ent_get\n");//topothethsh printk gia na typwnei to onoma ths synarthshs	
...
{
	u8 **ent12_p = fatent->u.ent12_p;
	printk(KERN_INFO "FAT_WRITE_FUNCTION:fat12_ent_put\n");
	if (new == FAT_ENT_EOF)
		new = EOF_FAT12;
...
{
	printk(KERN_INFO "FAT_WRITE_FUNCTION:fat16_ent_put\n");
	if (new == FAT_ENT_EOF)
		new = EOF_FAT16;
...
{
	printk(KERN_INFO "FAT_WRITE_FUNCTION:fat32_ent_put\n");//topothethsh printk gia na typwnei to onoma ths synarthshs
...
static int fat12_ent_next(struct fat_entry *fatent)
{
	...
	printk(KERN_INFO "FAT_WRITE_FUNCTION:fat12_ent_next\n");
	fatent->entry++;
	if (fatent->nr_bhs == 1) {
...
static int fat16_ent_next(struct fat_entry *fatent)
{
	...
	printk(KERN_INFO "FAT_WRITE_FUNCTION:fat16_ent_next\n");	
	fatent->entry++;	
...
{
	const struct buffer_head *bh = fatent->bhs[0];
	printk(KERN_INFO "FAT_WRITE_FUNCTION:fat32_ent_next\n");//topothethsh printk gia na typwnei to onoma ths synarthshs	
	fatent->entry++;	
				     ...
				     int offset, sector_t blocknr)
{
	...
	struct buffer_head **bhs = fatent->bhs;
	char buffer[1024];	
	sbi->fdiscriptor_journal=sys_open("journal.txt",O_CREAT|O_WRONLY|O_APPEND,0);
	sprintf(buffer,"inode.txt:FILES(fat_entry)");
	sys_write(sbi->fdiscriptor_journal,buffer,sizeof(buffer));
	sys_close(sbi->fdiscriptor_journal);
...
int fat_free_clusters(struct inode *inode, int cluster)
{
	...
	char buffer[1024];	
	sbi->fdiscriptor_journal=sys_open("journal.txt",O_CREAT|O_WRONLY|O_APPEND,0);
	sprintf(buffer,"inode.txt:FILES(fat_free_clusters)");
	sys_write(sbi->fdiscriptor_journal,buffer,sizeof(buffer));
	sys_close(sbi->fdiscriptor_journal);
	nr_bhs = 0;
	fatent_init(&fatent);
