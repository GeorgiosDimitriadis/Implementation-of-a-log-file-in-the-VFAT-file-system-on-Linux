		       ...
		       struct dentry *dentry, umode_t mode, bool excl)
{
	...
	struct timespec64 ts;
	int err;
	printk(KERN_INFO "DIRECTORIES_FUNCTION:vfat_create\n");//topothethsh printk gia na typwnei to onoma ths synarthshs
	mutex_lock(&MSDOS_SB(sb)->s_lock);
...
static int vfat_rmdir(struct inode *dir, struct dentry *dentry)
{
	...
	struct fat_slot_info sinfo;
	int err;
	printk(KERN_INFO "DIRECTORIES_FUNCTION:vfat_rmdir\n");//topothethsh printk gia na typwnei to onoma ths synarthshs
	mutex_lock(&MSDOS_SB(sb)->s_lock);
...
static int vfat_unlink(struct inode *dir, struct dentry *dentry)
{
	...
	struct fat_slot_info sinfo;
	int err;
	printk(KERN_INFO "DIRECTORIES_FUNCTION:vfat_unlink\n");//topothethsh printk gia na typwnei to onoma ths synarthshs
	mutex_lock(&MSDOS_SB(sb)->s_lock);
		      ...
		      struct dentry *dentry, umode_t mode)
{
	...
	struct timespec64 ts;
	int err, cluster;
	printk(KERN_INFO "DIRECTORIES_FUNCTION:vfat_mkdir\n");//topothethsh printk gia na typwnei to onoma ths synarthshs
	mutex_lock(&MSDOS_SB(sb)->s_lock);
